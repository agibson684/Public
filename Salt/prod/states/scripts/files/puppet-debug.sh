...
function debug {
set -x
puppet_apply_site="puppet apply site.pp --logdest console"
}
trap "bashtrap" EXIT
        bashtrap ()
        {
 ...
 orig_summary=/opt/puppetlabs/puppet/cache/state/last_run_summary.yaml
 new_summary=/root/last_run_summary.yaml
 puppet_apply_site="puppet apply site.pp --logdest syslog"
 ...
 cd /etc/puppetlabs/code/environments/production/manifests/ && git checkout $branch && git pull origin $branch && $puppet_apply_site
 cd /etc/puppetlabs/code/environments/production/manifests/ && git checkout $branch && git pull origin-remote $branch && $puppet_apply_site
 ...
 if [ "$1" = '--help' ]; then 
  ...
  echo  'master pulls the default maser branch from origin repo and does a puppet apply on the server.'
  echo  'exp pulls the experimental branch from origin repo and does a puppet apply on the server'
  echo 'debug is  on debuging mode'
  exit 0
 fi
 if [ "$1" = '-help' ]; then 
 echo   'please use the following commands'
